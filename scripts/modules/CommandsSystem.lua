...
-- Tools
function useerr(com)
	return "err:используйте: "..com.name..' '..(com.use or '');
end
...
function M.Execute(msg, other)
	...
	for key,value in ipairs(M.checks) do
		local resp = value(msg, args, other, comObj);
	...
	-- Execute
	local rmsg = { line = addline };
	local result = comObj.exe(msg, args, other, rmsg);
	...
	if res.message and string.starts(res.message, 'err:') then
		...
		for key,value in ipairs(M.onsuccess) do value(msg, args, other, comObj) end
	end
	return res;
end
